...
デフォルト: `docker compose down`（named volumeは保持される。データは消えない）。
  ...
  --reset-data  全named volume（storage-data, database-data, authentik-database-data,
                authentik-data, caddy-data, qdrant-data, embedding-model-cache,
                llm-model-cache）を完全に削除して停止する。元に戻せません。
...
if $reset_data; then
  ...
  log_warn "  storage-data database-data authentik-database-data authentik-data caddy-data qdrant-data embedding-model-cache llm-model-cache"
  if ! $assume_yes; then
    if [[ -t 0 ]]; then
