...
Chienamiは、研究室内の1台のデスクトップPCをサーバとして利用し、Notionのような操作感で研究室ノウハウを蓄積・整理・共有する知識基盤です。第一段階では知識蓄積機能だけを完成させ、Semantic Search、RAG/ローカルLLM、文書取り込み機能は後から独立して追加できる段階構成とします。研究データを外部クラウドへ送らないことを基本方針とします。
...
Phase 2（Chienami Search）: Outlineの公開済み知識をIndexerで索引化し、Embedding + QdrantによるSemantic Searchを追加する。検索結果は必ず元のOutlineページへ戻れるようにする。
Phase 3（Chienami AI）: Keyword Search、Reranker、ローカルLLMを追加し、RAGによる出典付き回答を提供する。根拠がない場合は回答を控える。
Phase 4（Document Import / Knowledge Circulation）: PDF、Word、PowerPoint、Markdown等を取り込み、Outline下書きとして生成する。研究者が確認・公開した後に検索/RAG対象へ反映する。
...
下図は将来の最終構成です。Phase 1では Reverse Proxy + Outline + PostgreSQL + Redis + 添付ファイル保存領域だけを同じデスクトップPC上で動作させます。Phase 2以降で検索・AI系コンテナを追加します。外部公開はせず、研究室LANからのみReverse Proxyへ接続できるようにします。
...
## 4.3 Qdrant: 検索用インデックス（Phase 2以降）
...
## 4.4 Embedding / Reranker（Phase 2 / Phase 3）

Embeddingは文章を「意味の近さ」で検索するための数値表現へ変換します。日本語・英語が混在する研究室を想定し、Phase 2の初期標準候補を Qwen/Qwen3-Embedding-0.6B とします。0.6B規模で、1024次元のEmbedding、32K context、多言語（100+言語）に対応しており、1台構成で扱いやすい候補です。Phase 3のReranker候補は Qwen/Qwen3-Reranker-0.6B とします。

最終モデルは研究室の実文書で評価して決定します。特に日本語、英語、数式、コード、装置名・モデル名の混在条件でRecallを比較します。

## 4.5 llama.cpp / Local LLM

Phase 3でLLMをllama.cppのHTTPサーバとして追加し、Chienami APIからのみアクセスします。llama.cppはOpenAI互換HTTP APIを提供し、GGUFモデルをローカル推論できます。GPU VRAMに応じて量子化モデルを選定します。Phase 1/2ではLLMは必須ではありません。

モデル名は更新が速いため、設計書では固定せず「日本語性能・指示追従・長文コンテキスト・商用/研究利用条件・GGUF提供状況」で選定します。

## 4.6 Document Importer（Phase 4 / 将来）
...
# 5. 検索・RAG処理フロー（Phase 2 / 3）


## 5.1 Semantic Search用索引作成フロー（Phase 2）
...
**↓**
**↓**
Phase 2は検索結果を返した時点で終了し、LLMを使用しません。Phase 3ではLLMに「根拠にない内容を断定しない」「不明な場合は不明と答える」「各主張の根拠をSource IDで示す」というsystem promptを設定します。
...
取り込みフロー: ファイル選択 → 本文抽出 → metadata / 重複確認 → Outline下書き生成 → 人間レビュー → 公開 → Phase 2/3のIndexerが索引更新

## AIによるノウハウ追加支援（Phase 4 / 将来）

Phase 4では、短いメモをAIに渡して上記テンプレートのOutline下書きを生成する機能も追加できます。AIが自動公開するのではなく、研究者による確認・修正・公開を必須にします。
...
Phase 1ではOutline自身の権限管理だけを使用します。Phase 2以降、別の検索索引を持つと「Outlineでは見えない文書が検索結果やAI回答から見える」危険が生じます。そのため、権限連携が完成するまでは研究室全員が閲覧できるCollectionだけを索引対象にし、制限付きCollectionを扱う場合はユーザー単位のアクセスフィルタを必須とします。
...
2) Phase 1のOutlineが正常に復旧したことを確認する。Phase 2/3を導入済みの場合のみIndexer・Chienami APIを起動する。
3) Phase 2以降でQdrantが失われていれば、Outline全件から再indexする。
4) Phase 3を導入済みの場合のみLLMモデルを復元または再配置する。
5) Phase 2/3を導入済みの場合のみ代表質問セットで検索結果と出典リンクを確認する。
...
2) Outline添付ファイルと、必要に応じてPhase 4の原本文書をアーカイブする。
...
6) Phase 2以降はQdrantを原則として新しい本番環境で再indexし、開発索引をコピーしない。

# 9. ハードウェア設計

Phase 1のOutline中心構成は比較的軽量で、GPUは不要です。Phase 2のEmbeddingもCPUで運用可能です。Phase 3のローカルLLMはGPUがあると応答速度が大きく改善しますが、24GB級GPUを必須要件にはしません。小型の量子化LLMなら8〜12GB VRAMから開始でき、予算に応じて16GB以上へ拡張します。同じPCを将来拡張する場合は、GPU搭載余地・電源容量・RAMスロット・NVMe増設余地を確保します。


## Phase 1 初期推奨

Phase 1だけなら高性能GPUや大規模CPUは不要です。Phase 3も最初は8〜12GB級GPUまたはCPU推論で小さく始め、実際の利用状況を見て増強できます。将来より大きなLLMや複数同時利用が必要になった場合に備え、GPU交換が可能なケース、電源、冷却、RAM増設余地を確保しておく方針とします。

# 10. 将来API仕様（Phase 2 / 3）

Phase 1では独自Chienami APIは必須ではなく、Outlineだけで運用します。以下はPhase 2の検索UIとPhase 3のAI画面から利用する将来のAPI境界です。UIを変更しても検索/RAG本体とのインターフェースを維持できるようにします。
...
Phase 2: Indexerの成功/失敗、対象document_id、処理時間、最終index時刻
Phase 2/3: 検索時間・取得件数、Phase 3ではrerank時間・LLM生成時間
エラー内容。ただし質問本文や機密文書本文を無条件にログへ残さない。
資源監視: ディスク、RAM、PostgreSQL容量。Phase 2以降はQdrant、Phase 3ではGPU VRAMも追加

## Phase 2 / 3 検索・RAG評価セット
...
## Phase 2: Chienami Search（Semantic Search）
...
## Phase 3: Chienami AI（RAG / Local LLM）
...
## Phase 4: Document Import / Knowledge Circulation（将来拡張）
...
## 将来の検索・AI機能（Phase 2 / 3）

1) Phase 2導入後はChienami Searchから意味検索できる。
...
**[9] Qwen3-Embedding-0.6B: https://huggingface.co/Qwen/Qwen3-Embedding-0.6B    Phase 2の多言語Embedding初期候補。0.6B、1024次元、32K context、100+言語、Apache-2.0。**
[10] Qwen3-Reranker-0.6B: https://huggingface.co/Qwen/Qwen3-Reranker-0.6B    Phase 3のReranker初期候補。0.6B、32K context、100+言語、Apache-2.0。
...
本設計書の次工程ではPhase 1（Chienami Knowledge）のみを実装対象とします。検索・RAG・文書取り込みはPhase 1の受入完了後に追加します。
